...
namespace KTWE4
{
   ...
   public class PInfo
   {
    ...
    public class KTWEServer
    {
        ...
        private RESTClient.Client newClient;
...

        {
            ...
            ReadConfig();
            newClient = new RESTClient.Client(WEBURL);
            var result = newClient.Login(WEBUSER, WEBPASSWORD, CLIENTSECRET, CLIENTID, SCOPE).Result;
            MiniLogString("Login to REST:" +result.token_type, 0);
        ...
        public string WEBVERSION;
         ...
         public void ReadConfig()
         {
            ...
            MiniLogString("Read config", 0);
             if (ConfigOK == false)
             {
                ...
                CLIENTID = node.Attributes.GetNamedItem("CLIENTID").Value;
                CLIENTSECRET = node.Attributes.GetNamedItem("CLIENTSECRET").Value;
         ...
         private CMD  MakeDecision(string str, int Ant, bool Reject)
         {
            ...
            if(newClient.NextRefreshTime.AddMinutes(-10) >= DateTime.Now)
            {
                MiniLogString("Refresh token",0);
                var result = newClient.Refresh(WEBUSER, WEBPASSWORD, CLIENTSECRET, CLIENTID, SCOPE).Result;
                MiniLogString("Result: " + result.token_type, 0);
            }
         ...
         private CMD MakeRequest(String Tag, String Oper, String Place, int CacheTime, int Ant)
         {
            ...
            if (Oper.ToLower() == "вернибилет")
            {
                ...
                addr = "readers?filter[query]=barcode%20" + Tag;
            }
            ...
            if (Oper.ToLower() == "вернибилет")
            {
                MiniLogString("get request to " + addr, 0);
                var result = newClient.Process(addr, "GET", "");
                MiniLogString(result, 0);

                var dyn = JsonConvert.DeserializeObject<dynamic>(result);
                try
                {
                    if (dyn.data[0].attributes.categoryOfReader.Value == "03")
                    {
                    ...
                    MiniLogString("categoryOfReader: " + dyn.data[0].attributes.categoryOfReader.Value, 0);
                }
                catch
            {
                t_resut = CMD.CMD_DENY;
                ...
                try
                {
                    if (dyn.data[0].attributes.endingOfPenalty.Value != "")
                    {
                        t_resut = CMD.CMD_DENY;
                    }
                    else
                    ...
                    MiniLogString("endingOfPenalty: " + dyn.data[0].attributes.endingOfPenalty.Value, 0);
                }
                catch {
                    t_resut = CMD.CMD_DENY;
                }
                ...
                else
                {
                    ...
                    string param = @"{""data"":{""type"":""visit"",""attributes"":{""visitId"":"""",""userCode"":""" + Tag + @""",""place"":""" + Place+@""",""operation"":""" + Oper + @"""}}}";

                    sVisitId = "";
                    try {
                    MiniLogString("post request to " + addr, 0);
                    var result = newClient.Process<string>(addr, "POST", param);
                    MiniLogString(result, 0);
                    var dyn = JsonConvert.DeserializeObject<dynamic>(result);
                        sVisitId = dyn.data.attributes.visitId.Value;
                    }
                    catch { }

                MiniLogString("VisitId: " + sVisitId, 0);
